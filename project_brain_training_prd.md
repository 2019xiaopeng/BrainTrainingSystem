...
### 5.1 关键方向的优先级与顺序建议（未来 5 件事）
**目标**: 在不牺牲稳定性与可运营性的前提下，按“安全治理 → 性能与规模 → 核心玩法 → 变现/留存 → 架构重构”的顺序推进。
- **优先级 1：管理员账号 + 管理后台（可运营与安全底座）**
  - 先把封禁、资产修正、运营开关、审计日志做出来，避免后续做玩法/AI 后缺少治理手段。
- **优先级 2：排行榜预聚合（含我的排名 + 快照表）**
  - 属于公共热点读接口，最容易成为瓶颈；通过索引 + 快照表 + TTL/定时刷新，优先把性能与成本压住。
  - 将“榜单展示开关”纳入运营配置，支持随时关闭/维护。
- **优先级 3：闯关地图（核心玩法形态升级）**
  - 把训练从“菜单选择”升级为“地图推进”，驱动留存与目标感；同时为道具/付费点提供更自然的入口。
- **优先级 4：AI 高级分析报告（高价值付费点）**
  - 先定义结构化输入（近 N 局数据、六维、错误模式、目标）、输出模板（建议、训练计划），再引入模型生成自然语言与个性化指导。
  - 权限/计费/风控需要依赖管理员后台与审计。
- **优先级 5：完整迁移 Next.js（重构级别）**
  - 迁移的主要收益是 SSR/SEO/首屏注入登录态与更强的缓存策略；但成本高、风险大。
  - 建议在核心产品闭环与运营能力具备后再迁移，或采用“内容页先 Next/SSG，训练主流程维持 SPA”的分阶段路径。

### 5.2 Vercel 部署注意事项（面向现有架构与未来 Next.js）
- **数据库连接与冷启动**
  - Serverless/Edge 环境下避免长连接耗尽；优先使用 Supabase/Neon 等具备连接池/数据代理的方案。
  - 重要接口（结算、购买、签到）要在超时/重试下保持幂等与事务原子性。
- **迁移与表结构更新**
  - Vercel 不会自动跑数据库迁移；需要在发布前/发布后通过 CI 或手动执行迁移（例如 drizzle migration）。
  - feature flags、leaderboard snapshots、audit logs 等表属于“平台能力”，要在上线前完成。
- **定时任务/快照刷新**
  - 快照表可以“按需刷新 + TTL”降低复杂度；若需要固定刷新（周榜/赛季榜），可用 Vercel Cron 或外部 Scheduler 调用刷新接口。
- **鉴权与后台安全**
  - 管理后台路由不等于安全，必须在服务端校验 `role`；同时建议限制来源（IP allowlist / 额外二次验证）并记录审计日志。
- **AI 相关（函数时长/成本/密钥）**
  - 模型调用在 Serverless 下要考虑超时、并发与成本；建议做队列/异步生成（先出结构化报告，再补充自然语言）。
  - 任何模型密钥只放服务端环境变量；输出需做内容过滤与注入防护（不要执行用户输入）。
