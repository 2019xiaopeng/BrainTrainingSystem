...
import { getHeader, requireAdmin } from "../../../_lib/admin.js";
...
const getUrl = (req: RequestLike): string => {
  ...
  return typeof raw === "string" ? raw : "http://localhost/api/admin/users/unknown/unban";
};
...
export default async function handler(req: RequestLike, res: ResponseLike) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await tx.update(user).set({ bannedUntil: null, bannedReason: null }).where(eq(user.id, id));
